...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const updateCardTransforms = useCallback(
    (passedScrollTop?: number) => {
      ...
      const scrollTop = passedScrollTop ?? currentScrollTop;
      ...
      cardsRef.current.forEach((card, i) => {
        ...
        // Only scale if baseScale is not 1 or itemScale is not 0
        const scale =
          baseScale === 1 && itemScale === 0
            ? 1
  ...
  const refreshOffsets = useCallback(() => {
    ...
    // Briefly reset transforms to get clean offsets
    const currentTransforms = cardsRef.current.map(
    ...
    // Restore transforms
    cardsRef.current.forEach((card, i) => {
      card.style.transform = currentTransforms[i];
    });
  ...
  return (
    ...
    >
      <div className="scroll-stack-inner min-h-screen pt-[20vh] pb-[50rem]">
        {children}
